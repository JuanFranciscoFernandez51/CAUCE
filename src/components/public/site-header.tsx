...
export function SiteHeader() {
  ...
  return (
    <div className="sticky top-0 z-40 px-3 pt-3 sm:px-4 sm:pt-4">
        ...
        <div className="flex h-14 items-center justify-between gap-3 pl-4 pr-2.5">
          ...
          <nav className="hidden items-center gap-1 md:flex" aria-label={t(lang, "nav.principal")}>
            {NAV.map((item) => (
              <Link
                key={item.href}
                href={item.href}
                className="rounded-full px-3.5 py-2 text-sm font-medium text-muted-foreground transition-colors hover:bg-muted hover:text-foreground dark:hover:bg-white/10"
              >
                {t(lang, item.clave)}
              </Link>
            ))}
          ...
          <div className="flex items-center gap-1">
            ...
            <div className="hidden md:flex">
              <Link
                href="/login"
                className="ml-1 inline-flex h-9 items-center rounded-full bg-foreground px-5 text-sm font-medium text-background transition hover:opacity-90"
              >
                {t(lang, "nav.entrar")}
              </Link>
            </div>
      ...
      {open ? (
        <nav
          className="mx-auto mt-2 max-w-5xl rounded-3xl border border-border bg-white p-3 shadow-[0_18px_50px_-16px_rgba(17,17,17,0.25)] dark:border-white/10 dark:bg-[#101319] md:hidden"
            ...
            {NAV.map((item) => (
              <li key={item.href}>
                <Link
                  ...
                  className="block rounded-2xl px-4 py-2.5 text-sm font-medium hover:bg-muted"
                >
                  {t(lang, item.clave)}
                </Link>
              </li>
