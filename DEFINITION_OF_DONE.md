...
- [ ] **Interacción directa y vistosa**: usable y sencillo, entrable de varias maneras. **Drag & drop** donde aplique (calendarios para reprogramar, kanban), **edición inline** en listas/campos (sin pantallas de solo-lectura), acciones rápidas desde varios lugares, y cuidado visual (estados, transiciones, vacíos lindos). Que sea cómodo, no solo funcional.
- [ ] **Entrega mínima = web front + back + automatizaciones, SIEMPRE**. Toda empresa recibe: (a) **web pública** —por template de rubro (institucional / catálogo / reservas / menú / portfolio), o **integración con su web existente** si ya tiene una que funciona—, (b) el **Cauce OS** (back administrativo), y (c) las **automatizaciones** corriendo en paralelo. No hay entregas sin capa web. El módulo `sitio` va activo por defecto.
- [ ] **Marca tomada del cliente**: el intake pide web/Instagram del negocio; de ahí se toma la identidad (colores, logo, estilo) para que el sistema y la web nazcan con SU marca, no con la de Cauce.
